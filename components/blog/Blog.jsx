...
const useScrollAnimation = (ref, callback) => {
  useEffect(() => {
...
const App = () => {
    ...
    // Circle Animation
    const scrollCircle = useCallback(() => {
      if (!circleRef.current) return;
      const { top } = circleRef.current.getBoundingClientRect();
      const scale = Math.min(Math.max(Math.abs(top) / window.innerHeight, 0), 1);
      circleRef.current.style.transform = `translate(-50%, -50%) scale(${
        top <= 0 ? scale : 0
      })`;
    }, []);
  
    // Discover Text Animation
    const scrollDiscover = useCallback(() => {
      if (!discoverContainerRef.current) return;
      const { bottom } = discoverContainerRef.current.getBoundingClientRect();
      const textTrans = Math.max(bottom - window.innerHeight, 0);
      const leftText = discoverContainerRef.current.querySelector(".text__left");
      const rightText =
        discoverContainerRef.current.querySelector(".text__right");
      if (leftText) leftText.style.transform = `translateX(${-textTrans}px)`;
      if (rightText) rightText.style.transform = `translateX(${textTrans}px)`;
    }, []);
    ...
    }, [animateVideo, animateProjects, scrollBlogPosts, scrollCircle, scrollDiscover]);
